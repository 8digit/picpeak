...
router.post('/:id/restore', adminAuth, requirePermission('archives.restore'), async (req, res) => {
  try {
    ...
    try {
      ...
      // Load photos manifest if present. The gallery filenames are renamed on upload,
      // so original_filename (and category) can't be derived from the extracted files —
      // they're only recoverable from a manifest the archive process wrote. Older archives
      // have no manifest, so this falls back to the gallery filename.
      const manifestByFilename = new Map();
      try {
        const manifestRaw = await fs.readFile(path.join(eventDir, 'photos_manifest.json'), 'utf8');
        for (const m of JSON.parse(manifestRaw)) {
          ...
          try {
            ...
            if (!existingPhoto) {
              ...
              extractedPhotos.push({
                event_id: archive.id,
                filename: filename,
                original_filename: manifestByFilename.get(filename)?.original_filename || filename,
                path: relativePath,
