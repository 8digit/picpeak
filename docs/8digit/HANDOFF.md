...
19. **Per-category download permissions (2026-05-27)** — Added `allow_downloads BOOLEAN DEFAULT true` to `photo_categories` (migration 075). Admin Settings → Categories shows a Download toggle per category. Backend enforces at single-photo download, download-all ZIP, download-selected ZIP, and secure image endpoint. All 7 frontend gallery layout components (Grid, Masonry, Mosaic, Timeline, Carousel, Premium, Story) now check `photo.category_allow_downloads !== false` before rendering download buttons and including slides in lightbox downloads. Event-level `allow_downloads` remains the master switch. Commits: `928164b`, `751ec75`.
...
- Latest custom migration: `075_add_category_allow_downloads.js`
